...
Outlier / spike fixes:
- Pseudorange plausibility filter (rejects obs outside 19–27 Mm)        [FIX]
- PDOP guard: epochs with PDOP > 10 are discarded (bad geometry)        [FIX]
- Earth-surface sanity check on solved ECEF position                    [FIX]
- Inter-epoch position jump limit (300 m/s max speed)                   [FIX]
- Warm-start poisoning prevention: prev_rx only updated on good epochs  [FIX]
- Residual rejection threshold tightened from 80 m to 40 m             [FIX]
...
def solve_epoch_position(
    ...
    residual_reject_threshold_m: Optional[float] = 40.0,
    ...
    # Pre-filter: drop pseudoranges that are physically impossible.
    # GPS satellites orbit at ~20,200 km; the one-way range to a receiver on
    # Earth is bounded roughly between 20,000 km and 25,700 km (nadir to
    # horizon).  Anything outside [19 000 km, 27 000 km] is a bad observation
    # (multipath cycle-slip artefact, bit error in the receiver, etc.) and
    # will destabilise the LS if included.
    PR_MIN_M = 19_000_000.0
    PR_MAX_M = 27_000_000.0
    sats = [s for s in sats if PR_MIN_M <= s.pseudorange_m <= PR_MAX_M]
    ...
    # -----------------------------------------------------------------------
    # Sanity check 1: solved position must be on or near Earth's surface.
    # Catches cases where the LS diverged to a nonsensical location.
    # -----------------------------------------------------------------------
    rx_norm = np.linalg.norm(rx)
    if rx_norm < (WGS84_A - 1_000_000) or rx_norm > (WGS84_A + 1_000_000):
        return None

    # -----------------------------------------------------------------------
    # Sanity check 2: PDOP must be reasonable.
    # A PDOP > 10 means the satellite geometry is so poor that the height
    # component is essentially unconstrained — these epochs produce the
    # "rockets to space" outliers visible in the KML.
    # -----------------------------------------------------------------------
    if len(H_rows) >= 4:
        try:
            H_last = np.vstack(H_rows)
            Q = np.linalg.inv(H_last.T @ H_last)
            pdop = math.sqrt(float(Q[0, 0] + Q[1, 1] + Q[2, 2]))
            if pdop > 10.0:
                return None
        except np.linalg.LinAlgError:
            return None
...
def solve_path(
    ...
    for idx, epoch in enumerate(epochs, start=1):
        ...
        # ------------------------------------------------------------------
        # Inter-epoch jump guard.
        # If we have a previous good fix, reject solutions that are physically
        # impossible given the elapsed time.  At 1 Hz a ground vehicle can
        # move at most ~100 m/s; we allow up to 300 m/s to cover aircraft and
        # brief GPS receiver clock jumps, but anything beyond that is an outlier.
        # When rejected, prev_rx is kept from the last GOOD epoch so the next
        # epoch still gets a sane warm-start.
        # ------------------------------------------------------------------
        MAX_SPEED_MPS = 300.0
        if prev_rx is not None:
            dt_epoch = (epoch.time_gps - solutions[-1].time_gps).total_seconds() if solutions else 1.0
            dt_epoch = max(dt_epoch, 0.01)
            jump_m = float(np.linalg.norm(rx - prev_rx))
            if jump_m > MAX_SPEED_MPS * dt_epoch:
                print(
                    f"[WARN] epoch {idx}/{len(epochs)} {epoch.time_gps.isoformat()} "
                    f"-> position jump {jump_m:.0f} m in {dt_epoch:.1f} s, discarded as outlier",
                    file=sys.stderr,
                )
                continue
        ...
        # Only use this epoch as the warm-start for the next one if its RMS
        # residual is plausible.  A very large RMS (> 200 m) usually means the
        # solver landed in a local minimum; keeping prev_rx from the last good
        # epoch gives the next epoch a better starting point.
        if rms < 200.0:
            prev_rx = rx
            prev_cb = cb
